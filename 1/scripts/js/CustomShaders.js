var CustomShaders = function(){
	...
	this.gridShader = {
		...
		fragmentShader: [
			...
			"vec4 color = mix(vec4(0.5+0.5*cos(time), 1.0 - uv.yx,0.5), texture2D(texture, vUv), (0.5 + 0.5*sin((-sin(time) + uv.x*sin(time)*2.0)*(-1.0 + uv.y*2.0)*5.0)));",
			// "vec4 color = mix(texture2D(texture, vUv), texture2D(texture, vUv)*1.9, (0.5 + 0.5*sin((-1.0 + uv.x*2.0)*(-1.0 + uv.y*2.0)*time*200.0)));",
			// "vec4 color = vec4(0.5+0.5*cos(time), 1.0 - uv.yx,1.0);",
			...
			// "		} else {",
			// "			color = vec4(uv.x,0.5+0.5*cos(time*3.0),uv.y,1.0)*0.5;",
			// "		}",
			// "	} else {",
			// "		color = vec4(1.0);",
			// "	}",
	...
	this.flowShader = {
		...
		fragmentShader: [
				...
				"    for( int i=0; i<20; i++ ){",
				"    	float h  = dot( texture2D(texture, uv*0.99         ).xyz, vec3(100.0) );",
				"    	float h1 = dot( texture2D(texture, uv+vec2(e.x,0.0)).xyz, vec3(100.0) );",
				"    	float h2 = dot( texture2D(texture, uv+vec2(0.0,e.y)).xyz, vec3(100.0) );",
